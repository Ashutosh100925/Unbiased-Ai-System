...
class AdmissionPredictor:
    def __init__(self):
        self.model_path = os.path.join(BASE_DIR, "models", "admission_model", "model.pkl")
        self.cols_path = os.path.join(BASE_DIR, "models", "admission_model", "columns.pkl")
        try:
            self.model = joblib.load(self.model_path)
            self.expected_cols = joblib.load(self.cols_path)
        except Exception as e:
            print(f"Warning: Could not load admission models. Ensure they are trained. {e}")
            self.model = None
    ...
    def predict(self, input_dict):
        ...
        # Preserve the original dict so fairness engine can use gender/caste
        ...
        decision = "ADMITTED" if prediction == 1 else "REJECTED"
