...
class LoanPredictor:
    def __init__(self):
        self.model_path = os.path.join(BASE_DIR, "models", "loan_model", "model.pkl")
        self.cols_path = os.path.join(BASE_DIR, "models", "loan_model", "columns.pkl")
        try:
            self.model = joblib.load(self.model_path)
            self.expected_cols = joblib.load(self.cols_path)
        except Exception as e:
            print(f"Warning: Could not load loan models. Ensure they are trained. {e}")
            self.model = None
